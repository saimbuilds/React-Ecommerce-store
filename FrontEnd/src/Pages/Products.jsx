      ...
      <div className="w-[20%]  h-fit min-h-[65%] bg-blue-100 text-black p-2 rounded-xl flex flex-col gap-4">
        ...
        <div className="flex justify-between">
          ...
          <button
            ...
            className="bg-blue-400 px-2 py-1 rounded-lg cursor-pointer text-2xl"
          >
            Add to Cart
          </button>
        </div>
        <Link
          ...
          className=" bg-blue-600 text-xl text-white px-2 py-1 rounded-lg cursor-pointer w-fit"
        >
          More Info
        </Link>
      </div>
  ...
  return (
    <>
      {products.length > 0 ? (
        ...
        <div className="bg-gray-800  flex flex-wrap gap-20 p-4 mt-10">
          {renderProduct}
        </div>
        </InfiniteScroll>
      ) : (
        <div className="h-screen w-screen bg-gray-800">Loading....</div>
      )}
    </>
  );
};
